...
Edit the HTML below and render it through `POST /render/html` to preview the accessible PDF. The
...
| `xmpSchemas` | — | Optional XMP extension schemas (namespace, prefix, name, properties) to declare in the PDF/A metadata, e.g. the Factur-X schema for an embedded e-invoice |
...
By default, `POST /render/html` returns the PDF body as `application/pdf`. Send
