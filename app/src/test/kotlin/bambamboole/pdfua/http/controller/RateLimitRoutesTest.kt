...
class RateLimitRoutesTest {
    ...
    @Test
    fun perIpLimitExceededReturns429() = testApplication {
        environment {
    ...
    @Test
    fun differentIpsHaveSeparateBuckets() = testApplication {
        environment {
            config = MapApplicationConfig(
    ...
    @Test
    fun globalCapAppliesAcrossIps() = testApplication {
        environment {
            config = MapApplicationConfig(
    ...
    @Test
    fun disabledAllowsUnlimitedRequests() = testApplication {
        environment {
            config = MapApplicationConfig(
    ...
    @Test
    fun healthIsNeverRateLimited() = testApplication {
        environment {
            config = MapApplicationConfig(
                "rateLimit.perIp" to "1",
