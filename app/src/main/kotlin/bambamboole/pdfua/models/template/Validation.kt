...
internal fun requireObject(
    ...
    if (value is JsonObject) {
        ...
        null to listOf(issue(path, ValidationCodes.INVALID_TYPE, "Expected object"))
    }
...
internal fun requireArray(
    ...
    if (value is JsonArray) {
        ...
        null to listOf(issue(path, ValidationCodes.INVALID_TYPE, "Expected array"))
    }
...
internal fun optionalStringField(
    ...
    return if (value is JsonPrimitive && value.isString && value !is JsonNull) {
        emptyList()
    } else {
        listOf(issue(path.child(key), ValidationCodes.INVALID_TYPE, "Expected string"))
    }
}
...
internal fun nullableStringField(
    ...
    return if (value is JsonPrimitive && value.isString) {
        ...
        listOf(issue(path.child(key), ValidationCodes.INVALID_TYPE, "Expected string or null"))
    }
}
                    ...
                    add(
                        issue(
                            ...
                            "Unknown font weight: '$token' " +
                                "(allowed: ${KNOWN_FONT_WEIGHTS.sorted().joinToString(", ")})",
                        ),
                    )
                }
            }
...
private fun validatePageConfig(
    page: PageConfig,
    path: ValidationPath,
): List<ValidationIssue> =
    buildList {
